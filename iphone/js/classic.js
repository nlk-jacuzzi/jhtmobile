...
//			});
	...
	if ( jQuery('.post.cats .tubs').size() > 0 ) {
		if ( jQuery('.post.post-8').size() > 0 ) {
			$('.post.cats .content').accordion({
				active: false,
				collapsible: true,
				heightStyle: "content"
			});
	...
	if ( jQuery('#dltopform').size() > 0 ) {
		...
		jQuery('#dltopform').each(function() {
			jQuery(this).bind({
				...
				jQuery.ajax({
				type: "POST",
				url: '/hot-tubs/dealer-locator/cities/',
				data: { "zipcodeSearch": 1, "data[Dealer][country_id]": 1, "zip": dzip },
				success: function(result) {
					// needs updating for new DL may 2014 update... :
					
					jQuery('#dlandimg').hide();
					jQuery('#dmap-canvas').remove();
					jQuery('#dlrap').show().children().empty();
					
					var dh3 = jQuery(result).find('.dealerheader');
					if ( dh3.size() > 0 ) {
						//console.log(dh3.html());
						var dinfo = dh3.next();
					} else {
						dh3 = jQuery('<h3>'+ jQuery(result).find('.dealer-banner-lt h2').html() + '</h3>');
						var dinfo = jQuery('<p />');
						jQuery(result).find('.dealer-details').find('p').each(function(i) {
							if ( i == 0 ) {
								dinfo.append('<br />' + jQuery(this).html())
								.find('.ac').html('<br />');
							}
							if ( i==1 ) {
								jQuery(this).children().prependTo(dinfo);
							}
						});
					}
					...
					// wrap dlphone
					var pho = dinfo.find('.phone,.phone2');
					pho.each(function() {
						var htm = jQuery(this).html();
						jQuery(this).html('<a href="tel:'+htm+'" onclick="_gaq.push([\'_trackEvent\', \'DealerLocator\', \'click-to-call\', \'Mobile\']);" >'+htm+'</a>');
					});
					...
					jht_geocoder.geocode( { 'address': daddr }, function ( results, status ) {
						if (status == google.maps.GeocoderStatus.OK) {
							...
							if ( status == 'ZERO_RESULTS' ) {
								status = 'No Jacuzzi Hot Tubs Dealers were found in your area.';
					...
					var dbtns = jQuery('<div class="dbtns"><a href="http://maps.google.com/?saddr=&daddr='+daddrp+'" onclick="_gaq.push([\'_trackEvent\',\'DealerLocator\',\'Directions\',\'Mobile\']);">Directions</a><a href="/hot-tubs/get-a-quote/" onclick="_gaq.push([\'_trackEvent\',\'DealerLocator\',\'Get-Quote\',\'Mobile\']);">Request a Quote</a></div>');
			...
			'dlloading': function() {
				jQuery('#dltopform input.dlsub').val('SEARCHING...').attr('disabled',true).addClass('offf');
			},
			'dlreset': function() {
				jQuery('#dlandimg').show();
				jQuery('#dmap-canvas').remove();
				jQuery('#promo, #dlrap').hide();
