...
var header=jQuery('#header');if(header.size()>0){header=header.get(0);header.addEventListener('touchmove',classicTouchMove,false);}if(jQuery('#tab-bar').length){var tabBar=jQuery('#tab-bar').get(0);tabBar.addEventListener('touchmove',classicTouchMove,false);}
...
jQuery('#dlzip').bind('focus',function(){if(jQuery(this).hasClass('locd')==false){jQuery(this).addClass('locd');navigator.geolocation.getCurrentPosition(jht_fposition);}});jht_geocoder=new google.maps.Geocoder();if(jQuery('#dltopform').size()>0){jQuery('#dlbtn').click(function(){navigator.geolocation.getCurrentPosition(jht_position);return false;}).trigger('click');jQuery('#dltopform').each(function(){jQuery(this).bind({'submit':function(){var dzip=jQuery('#dlzip').val();jQuery(this).trigger('dlloading');if(dzip!='Zip/Postal Code'){jQuery.ajax({type:"POST",url:'/hot-tubs/dealer-locator/cities/',data:{"zipcodeSearch":1,"data[Dealer][country_id]":1,"zip":dzip},success:function(result){jQuery('#dlandimg').hide();jQuery('#dmap-canvas').remove();jQuery('#dlrap').show().children().empty();var dh3=jQuery(result).find('.dealerheader');if(dh3.size()>0){var dinfo=dh3.next();}else{dh3=jQuery('<h3>'+jQuery(result).find('.dealer-banner-lt h2').html()+'</h3>');var dinfo=jQuery('<p />');jQuery(result).find('.dealer-details').find('p').each(function(i){if(i==0){dinfo.append('<br />' + jQuery(this).html()).find('.ac').html('<br />');};if(i==1){jQuery(this).children().prependTo(dinfo);}});}dh3.add(dinfo).appendTo('#dlresult');var pho=dinfo.find('.phone,.phone2');pho.each(function(){var htm=jQuery(this).html();jQuery(this).html('<a href="tel:'+htm+'" onclick="_gaq.push([\'_trackEvent\',\'DealerLocator\',\'click-to-call\',\'Mobile\']);" >'+htm+'</a>');});var daddr='';var dladdr1=dinfo.find('.address1');if(dladdr1.size()>0){daddr+=dladdr1.html();}
...
var dmapd=jQuery('<div id="dmap-canvas"></div>');dmapd.insertBefore('#dlrap');jht_geocoder.geocode({'address':daddr},function(results,status){if(status==google.maps.GeocoderStatus.OK){var mapOptions={zoom:14,center:results[0].geometry.location,scrollwheel:false,navigationControl:false,panControl:false,rotateControl:false,scaleControl:false,streetViewControl:false,zoomControl:false,draggable:false,disableDefaultUI:true,mapTypeId:google.maps.MapTypeId.ROADMAP};map=new google.maps.Map(document.getElementById('dmap-canvas'),mapOptions);map.setCenter(results[0].geometry.location);var marker=new google.maps.Marker({map:map,position:results[0].geometry.location});jQuery('#dltopform').trigger('dldone');}else{alert("Geocode was not successful for the following reason: "+status);if(status=='ZERO_RESULTS'){jQuery('#dltopform').trigger('dlreset');}}});var daddrp=daddr.replace(/ /g,"+");var dbtns=jQuery('<div class="dbtns"><a href="http://maps.google.com/?saddr=&daddr='+daddrp+'" onclick="_gaq.push([\'_trackEvent\',\'DealerLocator\',\'Directions\',\'Mobile\']);">Directions</a><a href="/hot-tubs/get-a-quote/" onclick="_gaq.push([\'_trackEvent\',\'DealerLocator\',\'Get-Quote\',\'Mobile\']);">Request a Quote</a></div>');dbtns.appendTo('#dlresult');jQuery(result).find('#truckd,#additional_html').appendTo('#dlresult');jQuery('#promo').show();}});}
return false;},'dlloading':function(){jQuery('#dltopform input.dlsub').val('SEARCHING...').attr('disabled',true).addClass('offf');},'dlreset':function(){jQuery('#dlandimg').show();jQuery('#dmap-canvas').remove();jQuery('#promo,#dlrap').hide();jQuery('#dlzip').val('').blur();jQuery(this).trigger('dldone');},'dldone':function(){jQuery('#dltopform input.dlsub').val('SEARCH').removeAttr('disabled').removeClass('offf');}});if(jQuery(this).hasClass('zipposted')){jQuery(this).submit();}});}}
